...
# 自动修复 PyCharm 运行环境（必须在其他 import 之前）
...
@app.before_request
def log_request():
    backend_logger.info(f"→ {request.method} {request.path} | {request.remote_addr}")

@app.after_request
def log_response(response):
    backend_logger.info(f"← {request.method} {request.path} → {response.status_code}")
    return response
...
# 前端构建产物目录
frontend_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'sau_frontend', 'dist')
...
def upload_file():
    ...
    try:
        # 保存文件到指定位置
        uuid_v1 = uuid.uuid1()
        backend_logger.debug(f"UUID v1: {uuid_v1}")
        ...
        file.save(filepath)
        backend_logger.info(f"📤 文件上传成功: {uuid_v1}_{safe_name}")
...
def getAccounts():
    ...
    try:
        with sqlite3.connect(Path(BASE_DIR / "db" / "database.db")) as conn:
            ...
            backend_logger.debug("📋 当前数据表内容（快速获取）")
            for row in rows:
                backend_logger.debug(str(row))
        ...
        backend_logger.error(f"获取账号列表时出错: {str(e)}")
        return jsonify({
            "code": 500,
        ...
        with sqlite3.connect(Path(BASE_DIR / "db" / "database.db")) as conn:
            ...
            if file_path.exists():
                try:
                    ...
                    backend_logger.info(f"✅ 实际文件已删除: {file_path}")
                except Exception as e:
                    backend_logger.warning(f"⚠️ 删除实际文件失败: {e}")
                    # 即使删除文件失败，也要继续删除数据库记录，避免数据不一致
            else:
                backend_logger.warning(f"⚠️ 实际文件不存在: {file_path}")
            ...
            if record.get('filePath'):
                ...
                if cookie_file_path.exists():
                    try:
                        ...
                        backend_logger.info(f"✅ Cookie文件已删除: {cookie_file_path}")
                    except Exception as e:
                        backend_logger.warning(f"⚠️ 删除Cookie文件失败: {e}")
...
def postVideo():
    ...
    # 打印获取到的数据（仅作为示例）
    backend_logger.info(f"📤 发布任务 | 平台: {type} | 标题: {title} | 文件数: {len(file_list)} | 账号数: {len(account_list)}")
    ...
    try:
        ...
        import traceback
        backend_logger.error(f"发布视频时出错: {str(e)}")
...
def upload_cookie():
    try:
        ...
        backend_logger.error(f"上传Cookie文件时出错: {str(e)}")
        return jsonify({
            "code": 500,
...
def download_cookie():
    try:
        ...
        backend_logger.error(f"下载Cookie文件时出错: {str(e)}")
        return jsonify({
            "code": 500,
...
def internal_error(e):
    import traceback
    backend_logger.error(f"500 错误: {str(e)}")
    traceback.print_exc()
    return jsonify({"code": 500, "msg": str(e), "data": None}), 500

if __name__ == '__main__':
    app.run(host='0.0.0.0', port=5409, debug=True, use_reloader=False)
