...
class TencentBaseUploader(BaseVideoUploader):
    ...
    async def wait_for_upload_complete(self, page: Page) -> None:
        _MAX_RETRIES = 150  # 2s sleep → 300s 总超时
        for _ in range(_MAX_RETRIES):
            try:
                ...
                tencent_logger.info(_msg("🏃", "正在上传视频中..."))
                await asyncio.sleep(2)
        raise RuntimeError(f"等待视频上传完成超时（{_MAX_RETRIES * 2}s）")

    async def submit_publish(self, page: Page) -> None:
        _MAX_RETRIES = 120  # 0.5s sleep → 60s 总超时
        for _ in range(_MAX_RETRIES):
            try:
            ...
            raise RuntimeError(f"等待发布成功跳转超时（{_MAX_RETRIES * 0.5:.0f}s），当前 url={page.url}")
