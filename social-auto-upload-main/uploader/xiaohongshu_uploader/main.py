...
_UPLOAD_MAX_RETRIES = 150   # 2s sleep → 300s 总超时
_PUBLISH_MAX_RETRIES = 120  # 0.5s sleep → 60s 总超时
...
class XiaoHongShuVideo(XiaoHongShuBaseUploader):
    ...
    async def upload_video_content(self, page: Page) -> None:
        ...
        for _ in range(_UPLOAD_MAX_RETRIES):
            try:
            ...
            raise RuntimeError(f"等待视频上传完成超时（{_UPLOAD_MAX_RETRIES * 2}s），当前 url={page.url}")
        ...
        for _ in range(_PUBLISH_MAX_RETRIES):
            try:
                if self.publish_strategy == XIAOHONGSHU_PUBLISH_STRATEGY_SCHEDULED:
            ...
            raise RuntimeError(f"等待视频发布成功跳转超时（{_PUBLISH_MAX_RETRIES * 0.5:.0f}s），当前 url={page.url}")
...
class XiaoHongShuNote(XiaoHongShuBaseUploader):
    ...
    async def upload_note_content(self, page: Page) -> None:
        ...
        for _ in range(_UPLOAD_MAX_RETRIES):
            ...
            raise RuntimeError(f"等待图文上传完成超时（{_UPLOAD_MAX_RETRIES}s），当前 url={page.url}")
        ...
        for _ in range(_PUBLISH_MAX_RETRIES):
            ...
            raise RuntimeError(f"等待图文发布成功跳转超时（{_PUBLISH_MAX_RETRIES * 0.5:.0f}s），当前 url={page.url}")
