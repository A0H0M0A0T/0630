...
_WAIT_MAX_RETRIES = 120   # 页面等待通用超时
...
class BaiJiaHaoVideo(object):
    ...
    async def upload(self, playwright: Playwright) -> None:
        ...
        # 等待页面跳转到指定的 URL
        for _ in range(_WAIT_MAX_RETRIES):
            ...
            baijiahao_logger.error("等待进入视频发布页面超时")
            raise TimeoutError("等待进入视频发布页面超时")
        ...
        for _ in range(_WAIT_MAX_RETRIES):
            ...
            baijiahao_logger.error("等待封面生成超时")
    ...
    async def uploading_video(self, page):
        for _ in range(_WAIT_MAX_RETRIES):
            ...
            baijiahao_logger.error("等待上传完成超时")
            return False
    ...
    async def set_schedule_publish(self, page, publish_date):
        for _ in range(_WAIT_MAX_RETRIES):
            ...
            baijiahao_logger.error("定时发布重试超时")
            raise TimeoutError("定时发布重试超时")
