...
_PUBLISH_MAX_RETRIES = 60   # 1s sleep → 60s 总超时
    ...
    async def upload(self, playwright: Playwright) -> None:
        ...
        try:
            ...
            for _ in range(_PUBLISH_MAX_RETRIES):
                ...
                raise RuntimeError(f"等待视频发布成功跳转超时（{_PUBLISH_MAX_RETRIES}s），当前 url={page.url}")
...
class KSNote(KSBaseUploader):
    ...
    async def upload_note_content(self, page: Page) -> None:
        ...
        for _ in range(_PUBLISH_MAX_RETRIES):
            ...
            raise RuntimeError(f"等待图文发布成功跳转超时（{_PUBLISH_MAX_RETRIES}s），当前 url={page.url}")
