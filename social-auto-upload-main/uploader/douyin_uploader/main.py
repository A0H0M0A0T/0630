...
_WAIT_SHORT_MAX_RETRIES = 120   # 0.5s sleep → 60s 总超时
_WAIT_LONG_MAX_RETRIES = 150    # 2s sleep → 300s 总超时
...
class DouYinVideo(DouYinBaseUploader):
    ...
    async def upload(self, playwright: Playwright) -> None:
        ...
        for attempt in range(_WAIT_SHORT_MAX_RETRIES):
            try:
            ...
            raise RuntimeError(f"等待进入发布页面超时（{_WAIT_SHORT_MAX_RETRIES * 0.5:.0f}s），当前 url={page.url}")
        ...
        for _ in range(_WAIT_LONG_MAX_RETRIES):
            try:
            ...
            raise RuntimeError(f"等待视频上传完成超时（{_WAIT_LONG_MAX_RETRIES * 2}s）")
        ...
        for _ in range(_WAIT_SHORT_MAX_RETRIES):
            try:
            ...
            raise RuntimeError(f"等待发布成功跳转超时（{_WAIT_SHORT_MAX_RETRIES * 0.5:.0f}s），当前 url={page.url}")
...
class DouYinNote(DouYinBaseUploader):
    ...
    async def upload_note_content(self, page: Page) -> None:
        ...
        for _ in range(_WAIT_SHORT_MAX_RETRIES):
            ...
            raise RuntimeError(f"等待进入图文发布页面超时（{_WAIT_SHORT_MAX_RETRIES * 0.5:.0f}s），当前 url={page.url}")
        ...
        for _ in range(_WAIT_SHORT_MAX_RETRIES):
            ...
            raise RuntimeError(f"等待图文发布成功跳转超时（{_WAIT_SHORT_MAX_RETRIES * 0.5:.0f}s），当前 url={page.url}")
