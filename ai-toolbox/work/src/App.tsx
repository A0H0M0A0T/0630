...
import { PenTool, Image as ImageIcon, Share2, LogOut, Film, AlertTriangle } from "lucide-react";
...
export default function App() {
  ...
  // Export from Hand-drawn Tab to copywriting (programmatic — bypasses confirmation)
  const handleExport = (title: string, creativeText: string) => {
    setCopyInitialFactors({ title, creativeText });
  ...
  if (!authChecked) {
    return <div className="min-h-screen bg-gray-100 flex items-center justify-center"><div className="w-8 h-8 border-4 border-gray-200 border-t-black rounded-full animate-spin" /></div>;
  }
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col font-sans select-none selection:bg-black selection:text-white" id="app-root">
      ...
      <header className="bg-white/80 backdrop-blur-md border-b border-gray-100 px-6 md:px-12 py-5 sticky top-0 z-50 flex flex-col md:flex-row items-center justify-between gap-4" id="main-header">
        ...
        <nav className="flex items-center flex-wrap justify-center gap-1 md:gap-4 text-sm font-medium text-gray-600" id="main-nav">
          {/* Workflow button — moved to first position (replaces AI 绘图) */}
          <button
      ...
      <main className="flex-grow flex items-center justify-center p-4 md:p-8" id="main-content">
        <div
          className="bg-white w-full max-w-6xl rounded-[2.5rem] overflow-hidden shadow-2xl border border-gray-100/50 flex flex-col md:flex-row min-h-[480px] transition-all"
          id="main-dashboard-container"
        >
          {/* All tabs are kept mounted — hidden with CSS, not unmounted.
              This preserves async operations (image gen, recognition, copywriting, workflow)
          ...
          <div style={{ display: activeTab === "workflow" ? "flex" : "none", width: "100%", flex: 1 }}>
            <WorkflowPage
              isActive={activeTab === "workflow"}
              onBusyChange={(busy: boolean) => handleTabBusyChange("workflow", busy)}
            />
          </div>
        </div>
      </main>
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md mx-4 text-center border border-gray-100">
            ...
            <p className="text-sm text-gray-500 leading-relaxed mb-6">
              当前 <strong className="text-gray-700">{
                generatingTab === "hand-drawn" ? "AI 绘图" :
                generatingTab === "recognition" ? "AI 识图" :
                generatingTab === "copywriting" ? "AI 文案" : "短视频方案"
